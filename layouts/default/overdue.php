  ...
  function render($rpt) {
    ...
    $lay = new Lay;
      $lay->pushFont('Helvetica', 10);
        $lay->container('Columns', ['margin-left'=>'1in', 'margin-right'=>'1in', 'margin-top'=>'1in', 'margin-bottom'=>'1in']);
          $mbr = NULL;
          $oldmbr = NULL;
          while ($row = $rpt->each()) {
            if ($row['mbrid'] != $oldmbr) {
              ...
              $oldmbr = $row['mbrid'];
              $lay->container('Column', ['margin-left'=>'3.25in']);
                $lay->container('TextLine');
                  $lay->text(date('m/d/Y'));
                $lay->close();
                $lay->element('Spacer', ['height'=>14]);
                $lines = [OBIB_LIBRARY_NAME, '101 1st Street', 'Busytown, IA 11111-2222', 'phone: '.OBIB_LIBRARY_PHONE, 'hours: '.OBIB_LIBRARY_HOURS];
                foreach ($lines as $l) {
                  $lay->container('TextLine');
                    $lay->text($l);
                  $lay->close();
              ...
              $lay->pushFont('Times-Italic', 12);
                $lay->container('Line');
                  $lay->container('TextLine', ['width'=>'1.5in', 'underline'=>1]);
                    $lay->text('Title');
                  $lay->close();
                  $lay->container('TextLine', ['width'=>'1.5in', 'underline'=>1]);
                    $lay->text('Author');
                  $lay->close();
                  $lay->container('TextLine', ['width'=>'1in', 'underline'=>1]);
                    $lay->text('Due Date');
                  $lay->close();
                  $lay->container('TextLine', ['width'=>'0.75in', 'underline'=>1]);
                    $lay->text('Days Late');
                  $lay->close();
                $lay->close();
              $lay->popFont();
            ...
            $lay->container('Line');
              $lay->container('TextLine', ['width'=>'1.5in']);
                $lay->text($row['title']);
              $lay->close();
              $lay->container('TextLine', ['width'=>'1.5in']);
                $lay->text($row['author']);
